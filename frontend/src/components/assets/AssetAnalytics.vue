<template>
  <Card class="p-4">
    <h3 class="text-sm font-medium text-theme-text-content mb-4">Analytics</h3>

    <div v-if="loading" class="space-y-3">
      <div v-for="i in 3" :key="i" class="h-16 bg-theme-active/30 rounded animate-pulse"></div>
    </div>
    ...
    <div v-else-if="analytics" class="space-y-4">
      <!-- Summary Stats -->
      <div class="space-y-2">
        <div class="bg-theme-content rounded-lg p-3">
          <div class="text-xs text-theme-text-muted">Total Assets</div>
          <div class="text-xl font-bold text-theme-text-content mt-0.5">
        ...
        <div class="bg-theme-content rounded-lg p-3">
          <div class="text-xs text-theme-text-muted">Inactive Assets</div>
          <div class="text-xl font-bold text-theme-text-content mt-0.5">
      ...
      <p v-if="(analytics.total_assets || 0) === 0" class="text-xs text-theme-text-muted pt-1">
      ...
      <div
        ...
        "
      >
        <h4 class="text-xs font-medium text-theme-text-content mb-2">By Environment</h4>
        <div class="space-y-1.5">
          ...
          >
            <span class="text-theme-text-muted truncate">{{ env || 'Unknown' }}</span>
            <div class="flex items-center space-x-1.5">
              <div class="w-20 bg-secondary-200 rounded-full h-1.5">
                <div
              ...
              <span class="text-xs font-medium text-theme-text-content w-6 text-right">{{
                count
              }}</span>
            </div>
          </div>
      ...
      <div
        ...
        "
      >
        <h4 class="text-xs font-medium text-theme-text-content mb-2">By OS</h4>
        <div class="space-y-1.5">
          <div
            ...
            class="flex items-center justify-between text-xs"
          >
            <span class="text-theme-text-muted truncate">{{ os || 'Unknown' }}</span>
            <div class="flex items-center space-x-1.5">
              <div class="w-20 bg-secondary-200 rounded-full h-1.5">
                <div
                  class="bg-primary-600 h-1.5 rounded-full"
      ...
      <!-- vCPU Stats -->
      <div v-if="hasVcpuStats" class="pt-2 border-t border-theme-border-default">
        <h4 class="text-xs font-medium text-theme-text-content mb-2">vCPU Statistics</h4>
        <div class="space-y-1.5">
          <div>
            <div class="text-xs text-theme-text-muted">Average vCPU</div>
            <div class="text-sm font-semibold text-theme-text-content">
              {{ formatVcpu(analytics.average_vcpu) }}
            </div>
          </div>
          <div>
            <div class="text-xs text-theme-text-muted">Total vCPU</div>
            <div class="text-sm font-semibold text-theme-text-content">
              {{ analytics.total_vcpu ?? 0 }}
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Last Updated -->
      <div class="text-xs text-theme-text-muted pt-2 border-t border-theme-border-default">
